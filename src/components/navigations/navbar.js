        ...
        <nav className={styles.navbarContent}>
          ...
          <div className={styles.navbarMenu}>
            <div ref={ref} className={styles.navbarSearch} onClick={() => setSearchOpen(true)} onKeyDown={handleSearch}>
              <Input id="search-query" isLabeled={false} radius="full" type="text" name="keyword" value={keyword} placeholder="Cari yang kamu butuhkan disini ..." onChange={(e) => setKeyword(e.target.value)} endContent={<Search />} />
            </div>
            {width > 772 ? (
              <Fragment>
                {!isLoggedin && <Button id="nav-login" radius="full" size="sm" buttonText="Masuk" onClick={() => navigate("/login")} />}
                {!isLoggedin && <Button id="nav-signup" radius="full" size="sm" bgColor="var(--color-hint)" buttonText="Daftar Gratis" onClick={() => navigate("/signup")} />}
                ...
                {isLoggedin && <Button id="user-nav-logout" radius="full" size="sm" bgColor="var(--color-hint)" buttonText="Keluar" onClick={handleLogout} />}
              </Fragment>
            ) : (
              <Fragment>
                {isLoggedin && <Image width="var(--pixel-40)" height="var(--pixel-40)" style={{ borderRadius: "50%", objectFit: "cover", objectPosition: "center", cursor: "pointer" }} src={avatarURL} onClick={() => navigate("/profil")} />}
                <Button id={menuOpen ? "close-drawer" : "open-drawer"} variant="hollow" subVariant="icon" radius="full" color="var(--color-primary)" iconContent={menuOpen ? <Close /> : <Burger />} onClick={() => setMenuOpen(true)} />
              </Fragment>
            )}
          </div>
        </nav>
